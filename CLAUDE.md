...
- **Static PNGs in `kgdunn/figures` stay generated by a matplotlib
  script in `process-improve/scripts/`.** The chapter shows plotly
  code; the embedded image is the committed matplotlib PNG. Only
  touch the script when the underlying analysis itself changes.
