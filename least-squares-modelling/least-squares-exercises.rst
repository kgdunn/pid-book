...
.. question::
	...
	-	Use software to fit a linear model that predicts the yield from these variables (the `data set is available from the website <http://openmv.net/info/bioreactor-yields>`_). See the `R tutorial <https://learnche.org/4C3/Software_tutorial>`_ for building linear models with integer variables in R.
