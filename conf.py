...
if out.returncode == 0 or out.returncode is None:
    str_out = out.communicate()[0].split(b'\n')
source = str_out[0].split(b':')
release = source[1].strip() + b'-' + source[2].split()[0][0:4]
release = release.decode() # required on Python 3.5 (including the conversions with "b" in the above strings)
...
# of the sidebar.
html_logo = 'textbook-logo-no-text-lowres.jpg'
