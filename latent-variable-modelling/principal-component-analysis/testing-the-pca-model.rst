...
The above outline is for the case when there is no missing data in a new observation. When there are missing data present in :math:`\mathbf{x}'_{\text{new}}`, then we require a method to estimate the score vector, :math:`\mathbf{t}_\text{new}` in step 3. Methods for doing this are outlined and compared in the paper by `Nelson, Taylor and MacGregor <https://literature.learnche.org/item/68/missing-data-methods-in-pca-and-pls-score-calculations-with-incomplete-observations>`_ and the paper by `Arteaga and Ferrer <https://literature.learnche.org/item/20/dealing-with-missing-data-in-mspc-several-methods-different-interpretations-some-examples>`_. After that, the remaining steps are the same, except of course that missing values do not contribute to the residual vector and the SPE.
