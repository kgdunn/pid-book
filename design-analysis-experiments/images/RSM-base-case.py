...
create_notes_version = False
if create_notes_version:
    # ------------------------------
    # Added code to get an additional figure in the notes
    fig_notes = Figure(figsize=(5, 5))
    rect = [0.15, 0.1, 0.80, 0.82]  # Left, bottom, width, height
    ax_notes = fig_notes.add_axes(rect, frame_on=True)
    levels = np.array([0, 100, 200, 300, 400, 500, 600, 700]) #30, 40, 50, 55, 60, 65, 68, 70, 71, 71.5, 72, 73, 75, 76])
    CS_notes = ax_notes.contour(X1, X2, Z, colors='#444444', levels=levels, linestyles='dotted',)
    ax_notes.clabel(CS_notes, inline=1, fontsize=10, fmt='%1.0f' )
    ax_notes.set_title('Contours of profit per day ($)', fontsize=16)
    ax_notes.set_xlabel('Temperature [K]', fontsize=16)
    ax_notes.set_ylabel('Substrate concentration [g/L]', fontsize=16)

    factorial = np.array([320, 330, 0.5, 1.0])
    t_lo, t_hi, s_lo, s_hi = factorial
    ax_notes.text(325, 0.75, "    Baseline", horizontalalignment='left', verticalalignment='center')
    baseline = factorial_response([t_lo, t_hi], [s_lo, s_hi])
    std_order = [baseline[0,0], baseline[0,1], baseline[1,0], baseline[1,1]]
    plot_factorial(t_lo, t_hi, s_lo, s_hi, ax=ax_notes, add_centerpoint=True, label_points=['1', '2', '3', '4'])
    ...
    CS_notes = ax_notes.contour((X1*range_T/2+start_T), (X2*range_S/2+start_S), Z, colors="#007700", levels=[0, 100, 200, 300, 400, 500, 600])
    ax_notes.clabel(CS_notes, inline=1, fontsize=10, fmt='%1.0f' )

    ax_notes.set_xlim([T_low, 335])
    ax_notes.set_ylim([S_low, 1.25])
    save_figure('RSM-base-case-with-first-factorial-notes.png', fig_notes)
