...
One can improve the aliasing structure of a design when switching all the signs of all factors from the first fraction. Switching the signs means that we take the complete design matrix of factor settings, and simply flip the signs to create the second fraction. A factor setting that was run at the low level in the first fraction is then run at a high level in the second fraction. This includes generated factors: imagine that **D = AB**, and we had **A** :math:`=-1`, **B** :math:`=+1` for a particular run in the first fraction. In the first fraction we would have **D** :math:`= (-1)(+1) = -1`, while in the second fraction it would simply be **D** :math:`=+1`, with **A** :math:`=+1` and **B** :math:`=-1` respectively, since we simply flip signs. **We do not regenerate the generated factors.**

In the :math:`2^{7-4}_\text{III}` example we ran 8 experiments, with the defining relation

.. centered:: **I = ABD = ACE = BCF = ABCG = BCDE = ACDF = CDG = ABEF = BEG = AFG = DEF = ADEG = CEFG = BDFG = ABCDEFG**

whose shortest words have three letters, which is why the main effects are aliased with two-factor interactions. Now run another 8 experiments with every sign flipped. When the two sets of 8 runs are analyzed together, a word remains a defining word only if it appears with the *same* sign in both halves. Flipping all the factor signs multiplies a word by :math:`(-1)` raised to the power of its length, so the odd-length words change sign and drop out, leaving only the even-length words. (The odd-length words are not lost; they become confounded with the fold, that is, with the split between the two fractions.) The combined 16-run design therefore has the defining relation

.. centered:: **I = ABCG = ABEF = ACDF = ADEG = BCDE = BDFG = CEFG**

with any three of these seven words serving as independent generators. Its shortest word now has four letters, so this is a :math:`2^{7-3}_\text{IV}` design: every main effect is aliased only with three-factor and higher interactions, clear of all two-factor interactions, while the two-factor interactions remain aliased with each other. Folding the whole design has bought a clean read on all seven main effects, at the cost of a second batch of 8 runs.
