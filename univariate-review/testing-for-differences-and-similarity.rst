...
We address the question of whether or not there was a *significant difference* between system A and B. A significant difference means that when system B is compared to a suitable reference, that we can be sure that the long run implementation of B will lead, in general, to a different yield (%). We want to be sure that any change in the 10 runs under system B were not *only due to chance*, because system B will cost us $100,000 to install, and $20,000 in annual software license fees.
	...
	*Note*: those with a traditional statistical background will recognize this section as one-sided :index:`hypothesis tests <single: hypothesis test>`. We will only consider tests for a significant increase or decrease, i.e. :index:`one-sided tests <single: hypothesis test; one-sided>`, in this section. Rather than casting the calculation as a formal hypothesis test with a fixed cut-off, we report the probability that the observed difference could have arisen by chance; the underlying calculation is the same. That probability lets you weigh the risk against the cost of the change, instead of returning a clear-cut yes/no answer.
...
.. _univariate_bayesian_two_sample:

A Bayesian complement: the probability that system B exceeds system A
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
The three routes above all end with a statement of the same shape: the probability that the
observed improvement of 3.04 units of yield could have arisen by chance. The section on
:ref:`Bayesian estimates and credible intervals <univariate_bayesian_credible_intervals>` supplies
a fourth, complementary route, which phrases its answer directly in terms of the two long-run
means. Recall the key object there: the posterior distribution, the probability distribution for a
parameter after the data have been taken into account.

State the scenario first. Use only the 20 experimental runs. Assume, as in the pooled-variance
calculation of :ref:`the internal estimate of spread <univariate_eqn_pooled_variance>`, that the
two systems share a common variance, and that the runs are independent and normally distributed.
Give the long-run means, :math:`\mu_A` and :math:`\mu_B`, and the shared variance, priors so wide
that they express no preference (the weak priors of that earlier section). The quantity of
interest is the difference :math:`\mu_B - \mu_A`.

With these ingredients, the posterior for :math:`\mu_B - \mu_A` works out to be a
:math:`t`-distribution with 18 degrees of freedom, centred at the observed difference
:math:`82.93 - 79.89 = 3.04` and scaled by :math:`\sqrt{s_P^2 \times 2/10} = 3.02`: the same
centre, spread, and degrees of freedom that appeared in the pooled-variance calculation. The
probability that system B's long-run mean exceeds system A's is the area of this posterior to the
right of zero:

.. math::

	p\left(\mu_B - \mu_A > 0 \,|\, \text{data}\right) = 84\%
...
.. code-block:: python
	...
	var_pooled = ((n_A - 1) * A.var(ddof=1)
	              + (n_B - 1) * B.var(ddof=1)) / dof
	scale = np.sqrt(var_pooled * (1 / n_A + 1 / n_B))

	p_gt_0 = 1 - stats.t.cdf(0, df=dof, loc=diff, scale=scale)
	print(f"p(mu_B - mu_A > 0 | data) = {p_gt_0:.3f}")  # 0.836
	p_gt_2 = 1 - stats.t.cdf(2, df=dof, loc=diff, scale=scale)
	print(f"p(mu_B - mu_A > 2 | data) = {p_gt_2:.3f}")  # 0.633

	d = np.linspace(-8, 14, 500)
	density = stats.t.pdf(d, df=dof, loc=diff, scale=scale)
	fig = go.Figure()
	fig.add_trace(go.Scatter(x=d, y=density,
	                         name="Posterior of the difference"))
	fig.add_trace(go.Scatter(x=d[d >= 0], y=density[d >= 0],
	                         fill="tozeroy",
	                         name="Area where B exceeds A: 84%"))
	fig.update_layout(xaxis_title="Difference in long-run yields [%]",
	                  yaxis_title="Density")
	fig.show()

.. figure:: ../figures/univariate/bayes-yield-difference-posterior.png
	:alt:	Generated by univariate/bayesian_univariate_figures.py in the figures repository
	:width: 750px
	:align: center
	:scale: 90

For this weak-prior, pooled-variance formulation the 84% is exactly :math:`100\% - 16.4\%`: the
same :math:`t`-distribution curve with 18 degrees of freedom, read from the other side. The two
readings answer differently phrased questions. The 16.4% is the probability of observing a
difference at least this large if system B has no effect; the 84% is the probability, given the
data and the weak priors, that system B's long-run mean really does exceed system A's. Placed
beside the three earlier routes (11%, 15.2% and 16.4%), this fourth route reaches the same
practical conclusion about the two systems from the same 20 runs.

Having the full posterior also answers other direct questions with no further machinery. For
example, if the improvement only pays for the cost of switching control systems when it exceeds 2
units of yield, then :math:`p\left(\mu_B - \mu_A > 2 \,|\, \text{data}\right) = 63\%`, calculated
as the posterior's area to the right of 2.

One caution on scope: the exact agreement with the pooled-variance route holds for the weak
priors used here. An informative prior on either mean, from experience with these control systems
elsewhere, would shift the posterior, and the 84% along with it.
